...
export default function FilterComponent(props) {
    ...
    const [firstPartMoods, setFirstPartMoods] = useState([
        [<WineBarIcon />, 'Romantic'],
        [<BusinessCenterIcon />, 'Busines'],
        [<GroupsIcon />, 'Groups'],
    ])
    const [secondPartMoods, setSecondPartMoods] = useState([
        [<CelebrationIcon />, 'Party'],
        [<ChildCareIcon />, 'Children'],
        [<TableBarIcon />, 'Local']
    ])
  return (
    <div>
      <div >
        <div style={{display: 'flex', alignItems: 'center', justifyContent: 'space-between'}}><div>Filters</div><Button onClick={props.onClose}><CachedIcon /></Button></div>
        <div style={{paddingLeft: '30px', paddingRight: '30px', paddingBottom: '30px'}}>
            ...
            <div>
                ...
                <div>
                    <ul style={{listStyleType: 'none', display: 'flex', padding: 0, justifyContent: 'space-evenly'}}>
                        <div style={{display: 'flex',  alignItems: 'center', flexDirection: 'column'}}>
                        {firstPartMoods.map(elem => {
                            return (
                                <li key={`${elem[1]}`} style={{width: '60px'}} >
                                    <button onClick={() => alert(elem[1])} style={{width: '80px'}}>{elem[0]} <div>{elem[1]}</div></button>
                                </li>
                            )
                        })}
                        </div>
                        <div style={{display: 'flex',  alignItems: 'center', flexDirection: 'column'}}>
                        {secondPartMoods.map(elem => {
                            return (
                                <li key={`${elem[1]}`} >
                                    <button onClick={() => alert(elem[1])} style={{width: '80px'}}>{elem[0]} <div>{elem[1]}</div></button>
                                </li>
